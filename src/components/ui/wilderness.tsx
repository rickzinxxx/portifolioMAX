...
export const ParallaxHero: React.FC<ParallaxHeroProps> = ({
  ...
  const updateLayers = (
    ...
    layerRefs.current.forEach((el, index) => {
      ...
      el.style.transform = `perspective(2300px) translateZ(${
        ...
        -xVal * speedX
      }px)) translateY(calc(-50% + ${yVal * speedY}px)) translateZ(0)`;
    });
    ...
    if (textRef.current) {
      ...
      textRef.current.style.transform = `perspective(2300px) translateZ(${
        ...
        -xVal * textSpeedX
      }px)) translateY(calc(-50% + ${textSpeedY * yVal}px)) translateZ(0)`;
    }
  };
  ...
  return (
    <div
      ...
      id={`parallax-container-${imageId}`}
    >
      <div className="absolute inset-0 z-[100] pointer-events-none bg-[radial-gradient(ellipse_at_center,rgba(0,0,0,0)_65%,rgba(0,0,0,0.85))]" />
      ...
      {title && (
        <div
          ...
          id={`parallax-text-${imageId}`}
        >
          <h1 className="font-black text-[20rem] leading-[0.8] max-lg:text-[15rem] max-md:text-[5.8rem] max-sm:text-[3.3rem] opacity-30">
            {title}
          </h1>
        </div>
      )}
...
export function ParallaxHeroDemo() {
  return <ParallaxHero title="WILDERNESS" />;
}
