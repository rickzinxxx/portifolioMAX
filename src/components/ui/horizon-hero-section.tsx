...
export const HorizonHeroSection = () => {
  ...
  return (
    <div ref={containerRef} className="hero-container cosmos-style relative w-full h-screen overflow-hidden">
      <canvas ref={canvasRef} className="hero-canvas absolute inset-0 w-full h-full" />
      
      {/* Side menu */}
      <div ref={menuRef} className="side-menu fixed left-8 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-8" style={{ visibility: 'hidden' }}>
        <div className="menu-icon flex flex-col gap-1.5 cursor-pointer">
          <span className="w-6 h-0.5 bg-white"></span>
          <span className="w-4 h-0.5 bg-white"></span>
          <span className="w-6 h-0.5 bg-white"></span>
        </div>
        <div className="vertical-text text-[10px] font-black tracking-[0.5em] uppercase [writing-mode:vertical-lr] rotate-180 opacity-50">SPACE</div>
      </div>

      {/* Main content */}
      <div className="relative z-10 h-full flex flex-col items-center justify-center pointer-events-none px-6">
        <h1 ref={titleRef} className="hero-title text-5xl md:text-8xl font-black italic tracking-tighter uppercase mb-8 cinema-text-shadow text-center leading-none">
          {Array.from("ENGENHARIA DE SOFTWARE").map((char, i) => (
        ...
        <div ref={subtitleRef} className="hero-subtitle text-center space-y-2 opacity-60">
          <p className="subtitle-line text-[10px] md:text-xs font-black uppercase tracking-[0.6em]">
            Creative Engineering & Digital Architecture
          </p>
        </div>
      </div>
      ...
      <div className="scroll-sections invisible">
       {[...Array(2)].map((_, i) => {
          return (
